...
export class ContactService {
    private resourceUrl = SITES_SERVER_API_URL + 'api/contacts';
    private resourceSearchUrl = SITES_SERVER_API_URL + 'api/_search/contacts';
    ...
    update(contact: Contact): Observable<EntityResponseType> {
        const copy = this.convert(contact);
        return this.http
            .put<Contact>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<Contact>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }
    ...
    query(req?: any): Observable<HttpResponse<Contact[]>> {
        ...
        return this.http
            .get<Contact[]>(this.resourceUrl, { params: options, observe: 'response' })
    ...
    search(req?: any): Observable<HttpResponse<Contact[]>> {
        const options = createRequestOption(req);
        return this.http
            .get<Contact[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Contact[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertItemFromServer(contact: Contact): Contact {
        ...
        copy.createdDate = this.dateUtils.convertDateTimeFromServer(contact.createdDate);
        copy.lastModifyDate = this.dateUtils.convertDateTimeFromServer(contact.lastModifyDate);
        return copy;
    }
    ...
    private convert(contact: Contact): Contact {
        ...
        copy.createdDate = this.dateUtils.toDate(contact.createdDate);

        copy.lastModifyDate = this.dateUtils.toDate(contact.lastModifyDate);
        return copy;
    }
}
