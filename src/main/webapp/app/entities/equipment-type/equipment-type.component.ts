...
export class EquipmentTypeComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.equipmentTypeService
                ...
                .subscribe(
                    (res: HttpResponse<EquipmentType[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: EquipmentType) {
        return item.id;
    }
    registerChangeInEquipmentTypes() {
        this.eventSubscriber = this.eventManager.subscribe('equipmentTypeListModification', response => this.loadAll());
    }
