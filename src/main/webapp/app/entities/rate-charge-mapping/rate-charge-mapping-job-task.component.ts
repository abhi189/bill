...
@Component({
    selector: 'jhi-rate-charge-mapping-job-task',
    templateUrl: './rate-charge-mapping.component.html',
    styles: [
        `
            #title {
                margin-bottom: -1rem;
...
export class RateChargeMappingJobTaskComponent implements OnInit, OnDestroy {
    currentAccount: any;
    rateChargeMappings: RateChargeMapping[];
    error: any;
    success: any;
    ...
    loadAll() {
        this.rateChargeMappingService
            .getRateChargeMappingsByTariffIdAndJobId({
                page: this.page - 1,
            ...
            .subscribe(
                (res: HttpResponse<RateChargeMapping[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: RateChargeMapping) {
        return item.id;
    }

    registerChangeInRateChargeMappings() {
        this.eventSubscriber = this.eventManager.subscribe('rateChargeMappingListModification', response => this.loadAll());
    }
