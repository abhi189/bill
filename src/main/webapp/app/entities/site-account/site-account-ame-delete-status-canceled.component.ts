    ...
    template: `
        <form name="deleteForm">
            <div class="modal-header">
                <h4 class="modal-title" jhiTranslate="billingWebApp.invoice.delete.notAllowed">Delete operation not allowed</h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" (click)="clear()">&times;</button>
            </div>
            <div class="modal-body">
                <jhi-alert-error></jhi-alert-error>
                <p>An AME with status {{ ameStatus }} can't be deleted</p>
            </div>
...
export class SiteAccountAmeDeleteStatusCanceledComponent {
    @Input() ameStatus;
