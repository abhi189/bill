...
export class UtilityComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.utilityService
                ...
                .subscribe(
                    (res: HttpResponse<Utility[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: Utility) {
        return item.id;
    }
    registerChangeInUtilities() {
        this.eventSubscriber = this.eventManager.subscribe('utilityListModification', response => this.loadAll());
    }
